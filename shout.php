...
if($_POST)
{
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		if(mysqli_query($sql_con,"INSERT INTO shout_box(user, message, ip_address) value('$username', lower('$message'),'$user_ip')"))
		{
			$msg_time = date('h:i',time()); // current time
...
$row['message'] = str_replace(array_keys($invalid), array_values($invalid), $row['message']);
 


			$msg_time = date('h:i',strtotime($row["date_time"])); //message posted time
			echo '<div class="shout_msg"><time>'.$msg_time.'</time><span class="username">'.$row["user"].'</span> <span class="message">'.$row["message"].'</span></div>';
		}
	}
	else
	{
		header('HTTP/1.1 500 Escribe un comentario');
    	exit();
	}
}